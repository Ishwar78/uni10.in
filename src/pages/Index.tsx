...
const Index = () => {
  ...
  // Default feature rows (titles only, link hum /shop hi rakhenge)
  const defaultFeatureRows: FeatureRowData[] = [
    {
      key: "tshirts",
      title: "T-SHIRTS",
      imageAlt: "T-Shirts Collection",
    },
    {
      key: "denims",
      title: "DENIMS",
      imageAlt: "Denims Collection",
    },
    {
      key: "hoodies",
      title: "HOODIES",
  ...
  // Fetch Feature Rows from settings (sirf title/alt lene ke liye)
  useEffect(() => {
    let ignore = false;
    (async () => {
      try {
        ...
        const rows = Array.isArray(json?.data?.featureRows)
          ? (json.data.featureRows as FeatureRowData[])
          : [];
        if (!ignore) {
          const merged = defaultFeatureRows.map((def, idx) => {
            const fromApi = rows[idx];
            return fromApi
              ? {
                  ...def,
                  ...fromApi,
  ...
  // Fetch New Arrivals (simple & fast)
  useEffect(() => {
    let ignore = false;
    (async () => {
      try {
        ...
        const { ok, json } = await api(
          `/api/products?sort=createdAt:desc&limit=${limit}`
        );
        if (!ok)
          throw new Error(json?.message || json?.error || "Failed to load");
        let list = Array.isArray(json?.data)
  ...
  // Fetch top categories and products for each category (parallel)
  useEffect(() => {
    let ignore = false;
    (async () => {
      try {
        ...
        const catIds = top.map((cat) => cat.slug || cat.name || "");
        ...
        let productsAll = Array.isArray(pre.json?.data)
    ...
    return (
      ...
      >
        ...
        <div
          ...
          style={{
            animationName:
              direction === "ltr" ? "prod-marquee-ltr" : "prod-marquee-rtl",
  ...
  return (
    // 🔒 Mobile pe horizontal scroll band, upar ke navbar/icons side me nahi jayenge
    <div className="min-h-screen bg-background overflow-x-hidden sm:overflow-x-visible">
      <Navbar />

      {/* Hammer / ticker line – header ke just niche, mobile + desktop dono */}
      <div className="sticky top-16 z-40 overflow-x-hidden">
        <NewsTicker />
      </div>

      {/* Hero Section – single static banner */}
      <section className="relative h-screen flex items-center justify-center overflow-hidden mt-16">
        ...
        <div className="relative z-10 text-center px-3 sm:px-4 max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center mt-16 sm:mt-0">
      ...
      {/* Feature Rows Section – yahan se /shop?category=... bhej rahe */}
            ...
            {topCats.map((c) => {
              ...
              return prod ? (
                ...
                >
                  ...
                  <div className="mt-2 sm:mt-3 text-[11px] sm:text-sm font-semibold text-center group-hover:text-primary break-words leading-tight px-1">
                    {c.name}
                  </div>
                </div>
              ) : (
